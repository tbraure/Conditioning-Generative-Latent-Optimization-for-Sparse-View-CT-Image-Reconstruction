...
if __name__ == '__main__':
    '''
        Reconstruction example on 5 patients from the LDCT test dataset
    ...
    # For LDCT test dataset R = 512 and N = 1089
    x = torch.load(f'{DB}/test.pt').to(torch.float)

    # test_sizes should contain the sizes of each patient in the test set
    # For LDCT test_sizes = torch.Tensor([137, 309, 284, 277, 82])
    # so that, test_sizes.sum() == 1089
    s = torch.load(f'{DB}/test_sizes.pt').cumsum(-1).to(torch.int).tolist()
    s.insert(0, 0)

    # Increasing batch sizes multiplied by duration (in epochs)
    # For instance, during the first 600 epochs the batch size will be 32
    batch_sizes = [32]*600 + [64]*200 + [128]*100 + [256]*50
